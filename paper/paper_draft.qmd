...
connections using weighted connectivity metrics (incoming and outgoing) and the ratio of
source-to-sink connectivity for each reef. Weighted incoming connectivity is calculated by
...
populations. Weighted outgoing connectivity is calculated as the product of outgoing
connectivity strength from a target reef and the mean timeseries absolute coral cover
(km^2^) of the target reef, representing the larval flow leaving the target reef population.
A limitation of these weighted connectivity metrics is that they do not incorporate the
proportion of the source reef's population that is reproducing and producing larvae, instead
taking the total coral cover area as an indicator of larval productivity. The ratio of
...
time series DHW, ratio of source-to-sink larval connectivity, weighted incoming
connectivity, weighted outgoing connectivity and reef carrying capacity (coral habitable
area).
...
Across six of the 29 bioregions reefs in the high cover cluster have lower mean DHW, Log-10
carrying capacity and Log-10 weighted outgoing connectivity values than reefs in other
clusters (@sfig-access-esm1-5-bioregion-dhwmean;
@sfig-access-esm1-5-bioregion-carrying-capacity;
@sfig-access-esm1-5-bioregion-weightedoutgoing). There were no consistent patterns across
bioregions in their reefs' Log-10 coral carrying capacity area and cluster assignment
(@sfig-access-esm1-5-bioregion-carrying-capacity). Across most bioregions there were not
consistent differences between clusters in connectivity metrics such as Log-10
source-to-sink connectivity ratio (@sfig-access-esm1-5-bioregion-logsosi) and Log-10
weighted incoming connectivity (@sfig-access-esm1-5-bioregion-weightedincoming).
...
feature importance scores, accounting for 25% and 16% predictive importance respectively.
Weighted incoming and outgoing connectivity (Log10) features account for \~13.6% feature
importance. The carrying capacity of a reef accounts for 12.3% feature importance. Reef
source-to-sink ratio (Log10) both had feature importance scores of \~10%.
...
Analyses for source-to-sink ratio and weighted incoming and outgoing connectivity metrics
...
cluster (@fig-ts-cluster-pdp-twoway-incoming). Two-way analyses, for depth and weighted
outgoing connectivity, revealed a slight interaction with the increased low-cluster
probability for reefs of high outgoing connectivity primarily concentrated in shallow reefs,
while deep reefs with high outgoing connectivity have the greatest probability of being
...
intervention sites [@hock2017]. While reefs with high outgoing connectivity levels were
found to have greater probability of being assigned to low cover clusters when assessed
individually (@fig-ts-cluster-pdp-twoway-incoming), within the context of reef depth, reefs
...
![Weighted outgoing connectivity (in base-10 log scale) for reefs within each bioregion,
separated into clusters assigned by time series clustering. ‘Low’ clusters (green) are
clusters with the lowest median area-relative cover between 2030 and 2060 within each
bioregion. ‘Medium’ clusters (orange) are clusters with intermediate median area-relative
cover between 2030 and 2060 within each bioregion. ‘High’ clusters (blue) are clusters with
the highest median area-relative cover between 2030 and 2060 within each bioregion. Solid
lines within each violin represent the median base-10 log of weighted outgoing connectivity
for each cluster and bands represent the distribution of connectivity values within each
cluster. Displays results for ACCESS-ESM1-5
GCM.](../figs/ACCESS-ESM1-5/bioregion/weighted_outgoing_conn_bioregion_violin.png){#sfig-access-esm1-5-bioregion-weightedoutgoing
fig-align="center" width="14.82cm"}