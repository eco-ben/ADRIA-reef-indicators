...
[@fig-access-esm1-5-depth-carbonate]. In contrast, log10-transformed total connectivity
strength ($\text{log10}(S)$) was found to have no correlation with the number of years a
reef maintains cover above the required thresholds
...
depth in metres in the left-hand panel, and log total connectivity strength in the
right-hand panel. Red points represent the mean number of years above an assumed carbonate
budget threshold across reefs. Deeper reefs are better able to maintain cover above the
indicative carbonate budget threshold, while there is no considerable relationship between
connectivity strength and carbonate budget maintenance. Displays results from median time
series using
