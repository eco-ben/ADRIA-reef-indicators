...
days each, coming to a total of 24 days of connectivity matrices. The values in these matrices indicate the probability of larval transfer between each pair of reefs in the GBR, herein referred to as connectivity strengths. Indicative connectivity
...
connections using weighted connectivity metrics (incoming and outgoing) and the ratio of source-to-sink
connectivity for each reef. Weighted incoming connectivity is calculated by summing the product of each connection strength incoming to a target reef and the mean timeseries absolute coral cover (km^2^) of each matching source reef. This metric provides an approximation of overall larval flow into a reef, with high values indicating the target reef is supplied high volumes of larvae via strong connections and low values indicating the target reef has weak (or no) incoming connections or is supplied by small source populations. Weighted outgoing connectivity is calculated as the product of outgoing connectivity strength from a target reef and the mean timeseries absolute coral cover (km^2^) of the target reef, representing the larval flow leaving the target reef population. A limitation of these weighted connectivity metrics is that they do not incorporate the proportion of the source reef's population that is reproducing and producing larvae, instead taking the total coral cover area as an indicator of larval productivity.
The ratio of source-to-sink connectivity ($O:I$) for a target
reef represents the strength of the reef's outgoing larval flow relative to it's incoming
larval flow. This is calculated as: $O:I = \frac{S_O/N_O}{S_I/N_I}$. Where S represents the sum of connectivity pobabilities, N represents the number of connections, O denotes outgoing connectivity from the target reef and I denotes incoming connectivity to the target reef. If a reef has no incoming connections, the $O:I$ equals the $S_O/N_O$ value. Incoming connections in this calculation, do not directly represent the
larval supply gained as the breeding populations from which connections are coming from are not
...
between Low, Medium and High clusters. These characteristics include reef median depth,
...
of source-to-sink larval connectivity and weighted incoming connectivity.

#### Partial Dependence Analysis

Partial Dependence Analysis was conducted to understand the relationship between features and bioregion-scale time series cluster predictions using a trained Random Forest
(RF) classifier [@friedman2001]. The analysis utilised the reef characteristics of interest
as the predictors, including the median reef depth, mean DHW for the given GCM, Log10 - weighted incoming connectivity, Log10 - weighted outgoing connectivity, and Log10 - outgoing-to-incoming connectivity ratio. The full dataset
...
99% accuracy on the training set and 62% on the test set.
...
distribution of all other representative reef characteristics in the test dataset. Additionally,
to assess possible interaction between reef depth and incoming connectivity, two-way partial
dependence analysis was performed on grids along the range of these two features.
...
High cluster, with Medium and Low exhibiting lesser probability of such periods of
...
similar temporal behaviour between Low and High clusters compared to other Management
areas (@stbl-cluster-cid) due to the low degree of coral cover overall
...
cluster to maintain coral cover levels around 20% to 2050. The highest peak DHW levels
...
Comparing across GCMs, it is common for reefs to switch between clusters, including from
Low to High and vice versa (@sfig-gcm-bioregion-cluster-assignment). Of 2429 reefs, 20%
(503) of reefs have consistent cluster assignment across all GCMs. Of the 503 consistently
...
between reef characteristics and assigned clusters were found to be similar at the
...
cluster not only had greater levels of coral recovery but displayed persistence to 2050
...
In many bioregions reefs in the Low cluster have the lowest correlation between coral cover
and inverse-DHW time series whereas reefs in the High cluster were found to have the
...
source-to-sink connectivity ratio (@sfig-access-esm1-5-bioregion-logsosi) and Log-10 weighted incoming connectivity (@sfig-access-esm1-5-bioregion-weightedincoming).
...
Weighted incoming and outgoing connectivity (Log10) features account for \~13.6% feature importance.
The carrying capacity of a reef accounts for 12.3% feature importance. Reef source-to-sink ratio (Log10) both had feature importance scores of \~10%.
...
Analyses for source-to-sink ratio and weighted incoming and outgoing connectivity metrics suggest that reefs with greater outgoing connectivity and lower incoming connectivity levels have higher probability of being clustered into the Low cluster category compared to other reefs. Additionally, two-way analyses revealed an interaction between reef depth and incoming connectivity in determining cluster category assignment, particularly at the high depth range (@fig-ts-cluster-pdp-twoway). Two-way analyses revealed that reefs at greater than 10m depth are unlikely to be clustered into the Low cluster category, regardless of their incoming connectivity levels. However, at greater than 10m depth reefs with higher incoming connectivity are more likely to be assigned to a High cluster, while reefs with low incoming connectivity are more likely to be assigned to a Medium cover cluster (@fig-ts-cluster-pdp-twoway).

Individual feature analysis for carrying capacity indicates that its influence on cluster assignment is concentrated at the low range
of predictor values (@fig-ts-cluster-pdp). This aligns with the right-skewed distribution
of carrying capacity across reefs for these three predictor variables. Beyond this low-end range, the
partial dependence becomes near constant, contrary to their
...
bioregions, reveals increasing performance as bioregion mean reef depth increases (@sfig-rf-predictor-perf). This highlights that there may be unexplained
variation between reefs in shallow bioregions that contributes to cluster assignment and reef cover outcomes but
...
probability of a reef from the test dataset being clustered into each category
...
![Partial dependence plot indicating the probability of cluster assignment along interacting values of median reef depth (metres) and Log10 weighted incoming connectivity. Colour indicates the probability of assignment to each cluster, with lighter colours indicating higher probability of assignment. Panel A shows the probability of assignment for the Low cover cluster, Panel B shows Medium cover cluster and Panel C shows assignment probability for the High cluster category. Probability values for each cluster are generated by the random forest classifier using each combination of depth and incoming connectivity values along the axes at 50 grid points. The actual distributions of depth and incoming connectivity values for reefs are displayed in panels D and E respectively.](../figs/ts_cluster_rf_twoway_depth_conn_pdp.png){#fig-ts-cluster-pdp-twoway fig-align="center"
width="14.82cm"}
...
that sustain coral cover above $\theta_{0}$ (@fig-access-esm1-5-depth-conn-carbonate; @sfig-access-esm1-5-mean-carb-budget).
...
substatially. Additionally, two-way partial dependence analysis revealed that high incoming connectivity is important for High cluster assignment for reefs at depths of \>10m. Considering this 10-15m depth range, and targetting well connected reefs, when assessing possible restoration sites
may reduce the exposure of planted and seeded corals to high thermal stress, while still
maintaining effective logistical access to sites. Although transient, and difficult to measure, properties such as reef connectivity are likely to increase uncertainty in intervention planning to a greater degree than environmental characteristics such as reef depth. Reef intervention planning is a complex
...
such as source-to-sink connectivity ratio and total connectivity strength. While there is interaction between incoming connectivity and depth in reef cluster prediction, according to available input data, there are few reefs in the GBR that occur at depths \>10m and have Log10 weighted incoming connectivity levels below 0 (@sfig-access-esm1-5-mean-carb-budget), resulting in low importance of connectivity as an individual feature for clustering across the GBR. Furthermore, our
...
1996-2002 [@bode2018] time periods, without considering the influence of reef depth on resilience. Using updated 1km^2^ resolution larval connectivity
across the whole GBR, with a limited temporal sample of particle flows, our results suggest
that larval connectivity alone may not be important or reef resilience and recovery, but should be considered in combination with more influential factors such as depth.
...
necessary when incorporating connectivity as a supplementary factor in reef resilience and intervention-planning analyses.
...
three reef clusters are labelled with qualitative-ordered clusters from Low, Medium and
...
![Scatter plot indicating reef median depth (metres) and Log10 weighted incoming connectivity for each reef. Point colours represent the mean number of years each reef maintains positive carbonate budgets across each $theta$ value from 10-20% live coral cover. Displays results for ACCESS-ESM1-5
...
cover between 2030 and 2060 within each bioregion. ‘High’ clusters (blue) are clusters with
the highest median area-relative cover between 2030 and 2060 within each bioregion. Solid
lines within each violin represent the median base-10 log of weighted incoming connectivity for
each cluster and bands represent the distribution of connectivity values within
