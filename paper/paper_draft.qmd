...
whole-of-GBR scale but were found to lead to identical conclusions about characteristics influencing cluster cover compared to management area and bioregion scale analyses, and so are not shown here. To
...
GCMs. Figure [-@fig-gcm-cover-trajectories] illustrates the temporal behaviour of reefs by
their categorisation (Low, Medium and High). Results are displayed at the Management area
scale to illustrate the spatial differences in cover trajectories. Of the GCMs explored,
NorESM2-MM has the lowest Equilibrium Climate Sensitivity (2.5°C) and reefs exhibit
continuing recovery and decline dynamics up to 2075 under this hypothetical climate future.

Unsurprisingly, recovery phases are predominantly seen in the reefs in the High category,
with Medium and Low exhibiting lesser probability of such periods of recovery. Generally,
recovery and increases in coral cover are visible in simulated results up to 2045. Under
NorESM2-MM, the Far Northern Management area were found to have the most similar temporal
behaviour between Low and High categories compared to other Management areas
[@tbl-cluster-cid] due to the low degree of coral cover [@fig-gcm-cover-trajectories].
...
**\[TI: Ben, Just noticed - we need to make the rows (Management areas) go from North to
South!\]**
...
and 61% are clustered into Low, Medium and High clusters respectively. High assignment
variability across GCMs is expected as spatial differences in a given GCM's DHW trajectory
result in reefs being impacted at different relative magnitudes within each Management
areas.
...
between reef characteristics **\[TI: Ben, I think this is what you meant by "contextual
analysis factors"? depth, connectivity, dhw, carrying capacity etc?\]** and reefs' assigned
categories were found to be similar at the bioregion scale **\[TI: Ben, please confirm my
changes/edits are correct\]**. Results are therefore only shown and discussed for
ACCESS-ESM1-5, an intermediate Equilibrium Climate Sensitivity scenario [@sfig-ecs]. Those
interested in results for the other assessed GCMs are referred to Grier and Iwanaga
[-@grier2025]. Across bioregions, reefs in the High category not only had greater levels of
coral recovery but displayed persistence to 2050 [@sfig-access-esm1-5-bioregion-timeseries].
Reefs in the High category were consistently found to have the greatest median reef depths
across bioregions [@fig-access-esm1-5-bioregion-depth].
...
and DHW whereas reefs in the High category were found to have the strongest correlation
(@sfig-access-esm1-5-bioregion-dhwcor). Conversely, there are no consistent patterns across
bioregions when comparing their temporal behaviour with respect to their reef area, relative
cover, and mean of DHWs (@sfig-access-esm1-5-bioregion-dhwmean). There were no consistent
differences across bioregions and GCMs between clusters in their reefs' initial proportion
of habitable area occupied by corals (@sfig-access-esm1-5-bioregion-initialproportion).
Across most bioregions there were not consistent differences between clusters in
connectivity metrics such as source-to-sink connectivity ratio
(@sfig-access-esm1-5-bioregion-logsosi) and total connectivity strength
(@sfig-access-esm1-5-bioregion-totalconnectivity).

**\[TI: Note, I'm not sure if this section is interesting enough to keep now... let's
discuss.\]**
...
into Low, Medium, or High. These include, for each of the 2429 reefs assessed, the
indicative median reef depth, the mean DHW experienced, its coral carrying capacity, total
connectivity strength and source-to-sink ratio. Reefs' bioregion membership was excluded
from this analysis due to its categorical nature providing limited interpretable
information, and is further justified by its low feature importance score of 10%, the lowest
of all predictors [-@fig-ts-cluster-pdp, Panel A]. Reef median depth and mean DHW level have
the highest feature importance scores, accounting for 25% and 23% predictive importance
respectively. The carrying capacity of a reef accounts for 14.3% feature importance.
Connectivity strength and source-to-sink ratio both had feature importance scores of \~13%.

Reefs have the greatest probability of being clustered into Low if shallow, and High at
greater depths, with medium cluster reefs exhibiting a lower, and stable, variation in
assignment probability across depth ranges [@fig-ts-cluster-pdp, Panel B]. A large increase
in the probability of a reef being categorised as High can be seen between 10 and 12 meters
depth. This is at the edge of what is feasible with current technologies and restoration
approaches **\[TI: Citation here\]**, but indicates a depth range that could be adopted to
maximise longevity and benefits of, for example, coral seeding activities. With regard to
DHWs, assignment probability is roughly equal across the three categories up to 6 DHW. A
sharp delineation can be seen \> 6 DHW when comparing High with other categories, with lower
assignment probability (\~25%) compared to Medium and Low (\~40%).

Carrying capacity, source-to-sink ratio and total connectivity strength and indicate that
their influence on cluster assignments is concentrated at the low range of predictor values
[@fig-ts-cluster-pdp]. This aligns with the right-skewed distributions of values across
reefs for these three predictor variables. Beyond this lower range, the partial dependence
for these reef characteristics becomes near constant, indicating that interactions with
other reef characteristics are occurring at these values which is also evidenced by their
intermediate feature importance scores.

**\[BG: Will add either predictor distributions to these sub plots, or just in a general
variable distribution plot in the supplementary material.\]**

![Feature importance scores (Panel A) of the Random Forest classifier, and Partial
Dependence Plots for reef characteristics of interest indicating the probability of a reef
being classified to one of the three categories (Panel B). Partial Dependence Plots for
bioregions were excluded as they were found to be
uninformative.](../figs/ts_cluster_rf_feature_analysis.png){#fig-ts-cluster-pdp
fig-align="center" width="9.2cm"}

## Reef carbonate budget

Reef median depth has a moderate positive correlation with the number of years a reef
maintains coral cover above the assumed carbonate budget threshold required to sustain live
coral cover ($\theta_{0}$). Greater median depth is a key characteristic defining reefs that
sustain coral cover above $\theta_{0}$ [@fig-access-esm1-5-depth-carbonate]. Unsurprisingly,
the number of years above $\theta_0$ substantially decreases as $\theta_0$ is increased, as
does the Spearman's correlation. This is likely due to an increasing number of reefs being
unable to maintain coral cover above $\theta_0$ as it is increased, with the distribution of
years above threshold becoming increasingly right-skewed
[@fig-access-esm1-5-depth-carbonate; @fig-access-esm1-5-conn-carbonate]. In contrast,
log10-transformed total connectivity strength ($\text{log10}(S)$) was found to have no
correlation with the number of years a reef maintains cover above the required thresholds
[@fig-access-esm1-5-conn-carbonate]. The correlation between $\text{log10}(S)$ and
$\theta_0$ slightly decreases as the assumed threshold for $\theta_0$ increases.
...
median reef depth level in metres. Red points represent the mean number of years above a
carbonate threshold across reefs. Displays results from median time series using
...
connectivity strength. Red points represent the mean number of years above a carbonate
threshold across reefs. Displays results from median time series using
ACCESS-ESM1-5.](../figs/ACCESS-ESM1-5/log_total_strength_carbonate_scatter.png){#fig-access-esm1-5-conn-carbonate
fig-align="center" width="14.82cm"}

**\[BG: I could combine these two plots into a single larger figure maybe, since it's quite
easy to see the contrasting depth vs connectivity patterns with colours even when zoomed
out.\]**
...
**\[TI note: Important to speak to interesting depth range at edge of where coral planting
might feasibly/viably occur with current approaches\]**
...
@steinmann2020]. The three reef clusters are labelled with qualitative-ordered categories
from Low, Medium and High based on their median cover relative to each other cluster over
the 2030-2060 period.The selected time frame prevents the classification from being overly
influenced by any steep declines in the initial years of simulations, and avoids the
post-2060 time period where reefs are projected to be under severe stress and in a declined
state [@bozec2025].
